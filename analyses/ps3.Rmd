...
d$hand.look.logit = logit(d$hand.look)
...
```{r hand time model}
  m1 = lmer(hand.look.logit ~ age * condition + (condition | subid), data = d)
  summary(m1)
```
...
ran_effects=ranef(m_mixed)$trial
ggplot
```
...
```{r minimal mixed}
m_min = glmer(correct ~ condition*age.group + (1|subj), family = "binomial", data = d)
summary(m_min)
```

...
```{r select}
d.tidy <- select(d, WorkerId, contains("Input"), contains("Answer"))
```
...
```{r spread}
d.wide <- spread(d.gather,name, cost)
head(d.wide)
...
```{r}
library(gridExtra)
h1 = ggplot(data = d.spread, aes(x = subset(d.spread, item == 'Dog')$Cost)) + geom_histogram() 
h2 = ggplot(data = d.spread, aes(x = subset(d.spread, item == 'Plasma')$Cost)) + geom_histogram(binwidth = 100) 
h3 = ggplot(data = d.spread, aes(x = subset(d.spread, item == 'Sushi')$Cost)) + geom_histogram() 
grid.arrange(h1,h2,h3)
```
It seems most estimates of costs land on the rounded estimate or 'rounded' numbers (like 2000, or 2500, or 8 or 7.5). We know that a third of the examples for each category have a rounded anchor, which may be responsible for the rounded estimates. We don't get a real picture of how the histograms relate to anchor condition.
...
```{r}
mean_sum = summarise(d.spread,mean(Cost,na.rm = TRUE), sum(Cost, na.rm = TRUE))
```
...
```{r}
group_by(d.spread,Input.condition,item) %>%
...
```{r}
pcts <- d.tidy %>%
  mutate(pct_change = (cost-anchor)/anchor*100 ) %>%
  group_by(condition,item) %>%
  summarise(pct_change =mean(pct_change, na.rm = TRUE))
```
...
z.scores <- d.tidy %>% 
            ...
            group_by(condition, item) %>%
            summarise(zmean = mean(z_score, na.rm = T))
```
...
```{r}
qplot(item, zmean, fill=condition,
      position="dodge",
      stat="identity", geom="bar",
      data=z.scores)
```
